...
class User{
    ...
    // SING UP USER
    function singUpUser($name, $email, $password){
        try{
            ...
            if(!empty($this->user_name) && !empty($this->user_email) && !empty($this->user_pass)){
                ...
                if (filter_var($this->user_email, FILTER_VALIDATE_EMAIL)) { 
                    ...
                    if($check_email->rowCount() > 0){
                        ...
                        $user_image = rand(1,32);

                        $this->hash_pass = password_hash($this->user_pass, PASSWORD_DEFAULT);
                        $sql = "INSERT INTO `users` (name, email, password, p_p) VALUES(:username, :user_email, :user_pass, :user_image)";
    ...
    function find_user_by_id($id){
        try{
            $find_user = $this->db->prepare("SELECT * FROM `users` WHERE user_id = ?");
